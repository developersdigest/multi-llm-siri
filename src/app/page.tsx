//1. Import required hooks and types from React
"use client";
import { useEffect, useState, useRef } from "react";
//2. Declare global interface for extending Window object
declare global {
  interface Window {
...
export default function Home() {
  ...
  //5. Ref hooks for speech recognition and timer
  const recognitionRef = useRef<any>(null);
  const silenceTimerRef = useRef<any>(null);
  //6. Function to get CSS class based on model and play state
  const getModelClassName = (model: string): string => (model === prevModel && isPlaying ? " prominent-pulse" : "");
  //7. Async function to send user input to the backend and handle response
  const sendToBackend = async (message: string, modelKeyword?: string): Promise<void> => {
    setIsLoading(true);
    if (modelKeyword) setPrevModel(modelKeyword);
    else if (!prevModel) setPrevModel("gpt-3.5");
    try {
      stopRecording();
      const response = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message, prevModel: modelKeyword }),
      });
      ...
      if (data.data && data.contentType === "audio/mp3") {
        ...
        audio.onended = () => {
          setIsPlaying(false);
          startRecording();
          if (data.model) setPrevModel(data.model);
        };
      }
    } catch (error) {
  ...
  //8. Function to render model selection bubbles
  const renderModelBubble = (model: string, displayName: string, bgColor: string): JSX.Element => (
    <div className={`flex flex-col items-center model-bubble text-center${getModelClassName(model)}`}>
      {isLoading && model === prevModel && <div className="loading-indicator"></div>}
      <div className={`w-48 h-48 flex items-center justify-center ${bgColor} text-white rounded-full`}>
        {displayName}
      </div>
    </div>
  );
  //9. Handler for processing speech recognition results
  const handleResult = (event: any): void => {
    ...
    silenceTimerRef.current = setTimeout(() => {
      ...
      setPrevModel(detectedModel || "gpt");
      sendToBackend(interimTranscript, detectedModel);
      setTranscript("");
    }, 2000);
  };
  //10. Function to start speech recognition
  ...
  //11. useEffect hook to clean up on component unmount
  useEffect(
    () => () => {
  ...
  //12. Function to stop speech recognition
  const stopRecording = () => {
    if (recognitionRef.current) recognitionRef.current.stop();
  };
  //13. Handler to toggle recording state
  const handleToggleRecording = () => {
    if (!isRecording && !isPlaying) startRecording();
    else if (isRecording) stopRecording();
  };
  //14. Main render method
  return (
